...
{{< layer-stack >}}
用户感知层（交互逻辑）|HMI 提示策略 · 接管引导 · 状态透明化 · 信任建设
||
功能协调层（功能逻辑）|场景调度 · 模式切换 · 舒适度优化 · 多源融合决策
||
安全兜底层（兜底措施）|地图/OSP · 物理极限约束 · ODD 边界检测 · 降级策略
||
端到端模型（AI 核心）|感知 · 预测 · 规划 · 控制
{{< /layer-stack >}}

<p style="text-align:center;color:var(--muted);font-size:0.9em;margin-top:-0.5em">▲ 上层兜住下层的体验下限 &nbsp;·&nbsp; ▼ 下层提供上层的能力上限</p>
...
{{< quadrant title="SOTIF 场景四象限" >}}
✅|Area 1: 已知安全场景 (Known Safe)|保持并扩展
||
🔧|Area 2: 已知不安全场景 (Known Unsafe)|已识别，需修复
||
📋|Area 3: 未知安全场景 (Unknown Safe)|需验证确认
||
⚠️|Area 4: 未知不安全场景 (Unknown Unsafe)|最危险，尚未发现
{{< /quadrant >}}

{{< note type="info" >}}
**SOTIF 的目标**：Area 4 → Area 2（发现未知危险）→ Area 1（修复）；Area 3 → Area 1（验证确认）。最终使 Area 4 足够小，残余风险可接受。
{{< /note >}}
